...
1. Agent A identifies need for collaboration
   ...
   ↓
2. Agent A sends NegotiationProposal to Agent B
   → Proposal status: "proposed"
   ↓
3. Agent B evaluates proposal
   → Option 1: Accept (status: "accepted")
   → Option 2: Reject (status: "rejected")
   → Option 3: Counter (status: "countered", new proposal created)
   ↓
4. If accepted: Task execution begins
   If rejected: Negotiation ends
   If countered: Return to step 3 with Agent A evaluating
```

**Multi-Agent Negotiation Flow:**
```
1. Coordinator Agent initiates negotiation context
   → Identifies required specialist agents
   ↓
2. Coordinator sends proposals to multiple agents
   → Parallel negotiation tracks
   ↓
3. Each agent responds independently
   → Some accept, some counter, some reject
   ↓
4. Coordinator evaluates responses
   → Adjusts terms based on feedback
   → Sends counter-proposals as needed
   ↓
5. Negotiation converges
   → All required agents accept terms
   → Collaborative task execution begins
```
