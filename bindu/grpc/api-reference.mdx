...
<Steps>
  ...
  <Step title="Execute">
    ...
    <CodeGroup>
      ...
      message HandleRequest {
        ...
        string task_id = 2;                 // reserved — not populated by GrpcAgentClient today
        string context_id = 3;              // reserved — not populated by GrpcAgentClient today
      }
    ...
    <Note>
      `task_id` and `context_id` are part of the wire contract, but
      [`GrpcAgentClient._build_request`](https://github.com/getbindu/Bindu/blob/main/bindu/grpc/client.py)
  ...
  <Step title="Maintain">
    ...
    SDKs typically send `Heartbeat` every ~30 seconds (the reference TypeScript SDK uses
    ...
    settings but has no consumer in `bindu/grpc/` today.
  </Step>
</Steps>
...
<Info>
  ...
  SDK implements the streaming RPC.
</Info>
