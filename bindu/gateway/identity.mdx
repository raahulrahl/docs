...
<Tip>
**Audit trail.** Each signed call's verification result is surfaced inline in the SSE stream — the `<remote_content>` envelope you saw in the quickstart carries a `verified="yes" | "no" | "unknown"` attribute. With `auth.type: "did_signed"` and a working Hydra, you'll see `verified="yes"` on successful calls; mismatches surface as `task.finished` with `state: failed`.
</Tip>
