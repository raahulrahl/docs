...
```bash
uv run python examples/gateway_test_fleet/joke_agent.py
```
...
| 4 | `task.artifact` | The agent replied. The text inside `<remote_content>` is the real answer. That envelope is there so the planner (and you) remember this is *untrusted* data. The `verified` attribute is `unknown` here because we used `auth.type: "none"`; with `did_signed` it would be `yes` or `no`. |
