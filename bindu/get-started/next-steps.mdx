...
- **`OPENAI_API_KEY not set`** — `.env` file isn't being loaded. Add `from dotenv import load_dotenv; load_dotenv()` at the top.
- **`ConfigError: 'author' is a required field`** — you typoed the key. Re-read [Config reference](/bindu/get-started/config-reference).
