...
description: "Copy 15 lines of Python, run it, watch your agent come online with a DID, an A2A endpoint, and a tunnel to the public internet."
---

Here's the deal. You've got `bindu` installed and an `OPENAI_API_KEY` lying around. The block below is the whole thing — copy it into `agent.py`, run it, and 60 seconds later you have a live agent at `http://localhost:3773`.
...
You should see a sunflower in your terminal and a DID that looks like `did:bindu:you@example.com:research_agent:<id>`. That's your agent. It's online.

## What just happened

Four things are doing work in those 15 lines. None of them are infrastructure you had to write.
...
**`config`** is the agent's passport. The `author` and `name` get baked into a [DID](/bindu/learn/did/overview) — a cryptographic identity nobody can fake. The `deployment.expose: True` flips on an FRP tunnel so the rest of the agent internet can reach your laptop without router config.
...
**`bindufy(config, handler)`** wires it all up. Behind it: A2A JSON-RPC routing, DID key generation, Ed25519 signatures on every response, OAuth2 scopes, x402 payment hooks, an A2A-compliant HTTP server on `:3773`, a public tunnel, and an agent card at `/.well-known/agent.json` so other agents can discover you. You did not write any of that. You're welcome.
